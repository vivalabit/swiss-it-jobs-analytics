...
from swiss_jobs.providers.swissdevjobs_ch.service import (
    SwissDevJobsChParserService,
    slugify_runtime_name,
)

PROJECT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_RUNTIME_DIR = str(PROJECT_ROOT / "runtime" / "swissdevjobs_ch")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse swissdevjobs.ch vacancies into a local SQLite database."
    )
    ...
    parser.add_argument(
        ...
        choices=["new", "search"],
        default="new",
        help="new: all active vacancies, search: term/location filtering over the live feed",
    )
    parser.add_argument(
        "--canton",
        default="",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="How many feed pages to keep after filtering (0 = all pages)",
    )
    parser.add_argument(
        "--include",
        action="append",
...
def _resolve_database_path(args: argparse.Namespace) -> str:
    ...
    return str(Path(DEFAULT_RUNTIME_DIR) / runtime_name / "swissdevjobs_ch.sqlite")
...
def _run_with_watch(service: SwissDevJobsChParserService, config: ClientConfig) -> int:
    while True:
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = SwissDevJobsChParserService()
    try:

...
class JobsChParserService:
    def __init__(
        ...
        *,
        ...
        self.http_client = http_client or JobsChHttpClient()
        self.runtime_root = (
            Path(runtime_root)
            if runtime_root
    ...
    def _run_config(self, config: ClientConfig) -> ClientRunResult:
        ...
        try:
            ...
            if not vacancies and successful_queries == 0:
                raise RuntimeError(
                    "jobs.ch is unreachable for all queries (network/DNS or temporary blocking)."
                )
    ...
    def _build_error_result(self, *, client_id: str, error: str) -> ClientRunResult:
        timestamp = utc_now_iso()
        return ClientRunResult(
            run_id=make_run_id(timestamp),
            client_id=client_id,
            timestamp=timestamp,
            effective_config=ClientConfig(client_id=client_id, name=client_id),
            errors=[error],
        )
    ...
    def _resolve_database_path(self, config: ClientConfig) -> Path:
        ...
        if config.client_config_path:
            ...
            return self.runtime_root / slugify_runtime_name(stem) / "jobs_ch.sqlite"
        return self.runtime_root / slugify_runtime_name(config.client_id) / "jobs_ch.sqlite"


def run_jobs_ch_parser(run_config: ClientConfig | Mapping[str, Any]) -> ClientRunResult:
    return JobsChParserService().run(run_config)

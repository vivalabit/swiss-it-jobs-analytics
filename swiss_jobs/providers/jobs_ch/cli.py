...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse jobs.ch vacancies into a local SQLite database."
    )
    parser.add_argument(
        "--config",
        default="",
        help="JSON config path for a single run profile",
    )
    parser.add_argument(
        "--mode",
        choices=["new", "search"],
    ...
    parser.add_argument(
        "--location",
        default="",
        help="Location (for --mode search), e.g. zurich",
    )
    parser.add_argument(
        "--locations",
        action="append",
    ...
    parser.add_argument(
        "--database-path",
        default="",
        help="SQLite database path for run storage",
    )
    parser.add_argument(
        "--cookies-file",
        default="",
        help="Optional Netscape cookies.txt file for authenticated jobs.ch scraping",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
...
def _resolve_database_path(args: argparse.Namespace) -> str:
    ...
    return str(Path(DEFAULT_RUNTIME_DIR) / runtime_name / "jobs_ch.sqlite")
...
def _print_multi_result(results: list[ClientRunResult], *, as_json: bool) -> None:
    if as_json:
        payload = [
            {

...
class JobupChParserService:
    def __init__(
        ...
        *,
        ...
        self.http_client = http_client or JobupChHttpClient()
        self.runtime_root = (
            Path(runtime_root)
            if runtime_root
            else Path(__file__).resolve().parents[3] / "runtime" / "jobup_ch"
        )
    ...
    def _run_config(self, config: ClientConfig) -> ClientRunResult:
        ...
        try:
            ...
            if not vacancies and successful_queries == 0:
                raise RuntimeError(
                    "Jobup Switzerland is unreachable for all queries (network/DNS or temporary blocking)."
                )
    ...
    def _resolve_database_path(self, config: ClientConfig) -> Path:
        ...
        if config.client_config_path:
            ...
            return self.runtime_root / slugify_runtime_name(stem) / "jobup_ch.sqlite"
        return self.runtime_root / slugify_runtime_name(config.client_id) / "jobup_ch.sqlite"


def run_jobup_ch_parser(run_config: ClientConfig | Mapping[str, Any]) -> ClientRunResult:
    return JobupChParserService().run(run_config)

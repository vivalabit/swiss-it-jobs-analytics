...
from .client import JobupChHttpClient

PROJECT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "swiss_jobs" / "providers" / "jobup_ch" / "configs" / "config_info.json"
DEFAULT_DATABASE_PATH = PROJECT_ROOT / "runtime" / "jobup_ch" / "main-config" / "jobup_ch.sqlite"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Backfill salary/detail fields for already stored jobup vacancies.",
    )
    parser.add_argument(
        "--config",
        default=str(DEFAULT_CONFIG_PATH),
        help="Optional jobup config JSON used to resolve defaults such as cookies_file.",
    )
    parser.add_argument(
        "--database-path",
        default=str(DEFAULT_DATABASE_PATH),
        help="SQLite database path with stored jobup vacancies.",
    )
    parser.add_argument(
        "--cookies-file",
        default="",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Process all stored jobup vacancies, not only rows missing salary fields.",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
...
def backfill_database(
    ...
    *,
    ...
    client = http_client or JobupChHttpClient()
...
def _row_to_vacancy(row: sqlite3.Row) -> VacancyFull:
    return VacancyFull(
        ...
        source=str(row["source"] or "jobup.ch"),
    )

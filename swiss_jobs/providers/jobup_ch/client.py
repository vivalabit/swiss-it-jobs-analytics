...
class JobupChHttpClient:
    ...
    def enrich_vacancies(
        ...
        *,
        ...
        if show_progress:
            print(
                f"[progress] fetching jobup detail pages for {limit} vacancies with {detail_workers} workers...",
                file=sys.stderr,
            )
    ...
    def _fetch_query(
        ...
        *,
        ...
        while True:
            url = self._build_search_url(mode=mode)
            ...
            jobs, discovered_pages = parse_jobs_from_search_page(
                response.text,
    ...
    def _build_search_url(self, *, mode: str) -> str:
        if mode == "new":
            return f"{self.base_url}/en/new-vacancies/"

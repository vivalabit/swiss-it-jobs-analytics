...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=["new", "search"],
        default="search",
        help="new: latest Swiss vacancies, search: term/location search page",
    )
    parser.add_argument("--canton", default="", help="Swiss canton code, e.g. zh")
    ...
    parser.add_argument("--location", default="", help="Location, e.g. Zurich, Switzerland")
    parser.add_argument(
        "--locations",
        action="append",
        default=[],
    ...
    parser.add_argument("--max-pages", type=int, default=1, help="How many pages to scan")
    parser.add_argument("--include", action="append", default=[], help="Include token")
    parser.add_argument("--exclude", action="append", default=[], help="Exclude token")
    ...
    parser.add_argument("--skip-detail-schema", action="store_true", help="Skip vacancy detail enrichment")
    parser.add_argument("--detail-limit", type=int, default=10, help="Max detail pages per run")
    parser.add_argument("--detail-workers", type=int, default=1, help="Kept for config compatibility")
    parser.add_argument("--no-progress", action="store_true", help="Hide progress logs in stderr")
    parser.add_argument("--role-keywords", action="append", default=[], help="Target role keywords")
    parser.add_argument("--seniority-keywords", action="append", default=[], help="Target seniority keywords")
    parser.add_argument(
...
def _resolve_database_path(args: argparse.Namespace) -> str:
    ...
    return str(Path(DEFAULT_RUNTIME_DIR) / runtime_name / "linked_in.sqlite")
...
def _run_with_watch(service: LinkedInParserService, config: ClientConfig) -> int:
    while True:

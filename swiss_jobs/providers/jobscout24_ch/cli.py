...
from swiss_jobs.providers.jobscout24_ch.service import (
    JobScout24ChParserService,
    slugify_runtime_name,
)

PROJECT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_RUNTIME_DIR = str(PROJECT_ROOT / "runtime" / "jobscout24_ch")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse JobScout24 Switzerland vacancies into a local SQLite database."
    )
...
def _resolve_database_path(args: argparse.Namespace) -> str:
    ...
    return str(Path(DEFAULT_RUNTIME_DIR) / runtime_name / "jobscout24_ch.sqlite")
...
def _run_with_watch(service: JobScout24ChParserService, config: ClientConfig) -> int:
    while True:
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = JobScout24ChParserService()
    try:

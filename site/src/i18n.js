export const DEFAULT_LANGUAGE = "en";

export const LANGUAGE_OPTIONS = [
  { code: "en", label: "English", shortLabel: "EN" },
  { code: "de", label: "Deutsch", shortLabel: "DE" },
  { code: "fr", label: "Français", shortLabel: "FR" },
  { code: "it", label: "Italiano", shortLabel: "IT" },
];

export const LANGUAGE_LOCALES = {
  en: "en-CH",
  de: "de-CH",
  fr: "fr-CH",
  it: "it-CH",
};

export const LANGUAGE_COPY = {
  en: {
    documentTitle: "Swiss IT Job Market",
    languageSwitcher: {
      label: "Language",
      ariaLabel: "Select language",
    },
    common: {
      live: "Live",
      more: "More",
      less: "Less",
      allInputsPresent: "All expected CSV inputs are present.",
      moreFiles: (count) => `+${count} more`,
      na: "n/a",
    },
    sections: {
      overview: "Overview",
      snapshot: "Snapshot",
      findings: "Findings",
      "vacancy-trends": "Trends",
      "vacancy-map": "Map",
      charts: "Charts",
      experience: "Experience",
      salary: "Salary",
      skills: "Skills",
      metadata: "Methodology",
    },
    status: {
      loadingTitle: "Loading public snapshots",
      loadingMessage: "Fetching Swiss IT market statistics from generated JSON exports.",
      errorTitle: "Snapshot data unavailable",
      errorFallback: "The public snapshots could not be loaded.",
      fetchError: "Failed to load public statistics snapshots.",
    },
    hero: {
      titleAccent: "Swiss",
      titleRest: "IT Job Market",
      text:
        "Track hiring volume, salary benchmarks, and location hotspots from the latest public Swiss IT vacancy data.",
      dashboardCta: "Explore the dashboard",
      methodologyCta: "Read methodology",
      vacanciesTracked: "Vacancies tracked",
      published30d: (count) => `${count} published in 30 days`,
      currentCoverage: "current market coverage",
      medianSalary: "Median salary",
      salaryListings: (count) => `${count} listings with salary data`,
      salaryCoverage: "salary benchmark coverage",
      topHiringCity: "Top hiring city",
      activeVacancies: (count) => `${count} active vacancies`,
      cityDemand: "city demand",
    },
    snapshot: {
      kicker: "Snapshot context",
      title: "What this page is built on",
      description:
        "Quick context on freshness, sample size, coverage, and what the public export does not claim to measure.",
      sources: "Sources",
      sourceTitle: (count) => `${count} public job boards`,
      sourcesDescription: "Deduplicated at vacancy level before the public aggregate is published.",
      updated: "Updated",
      sampleSize: "Sample size",
      directEmployers: (count) => `${count} direct employers after agency filtering.`,
      salaryCoverage: "Salary coverage",
      salaryCoverageDescription: (count) =>
        `${count} listings with normalized CHF yearly salary data.`,
      includes: "Snapshot includes",
      coreSignals: "Core market signals",
      limitations: "Main limitations",
      limitationsTitle: "Read before comparing numbers",
      scopeItems: [
        "Vacancy volume",
        "Salary benchmarks",
        "Role and seniority mix",
        "City and canton demand",
        "Work mode split",
        "Skill and stack trends",
      ],
      limitationItems: [
        "Public aggregate snapshot, not a full census of the Swiss market.",
        "Salary benchmarks only use vacancies with explicit pay ranges.",
        "Salary rankings hide groups with fewer than 10 usable salary records.",
        "Coverage is limited to vacancies published from 2026 onward.",
        "Some fields are normalized or AI-assisted from posting text and structured metadata.",
      ],
    },
    findings: {
      kicker: "Key findings",
      title: "What stands out right now",
      location: "Location",
      locationTitle: (city) => `${city} keeps the largest share`,
      locationDescription: (share) => `${share} of tracked vacancies are clustered there.`,
      momentum: "Momentum",
      momentumTitle: (role) => `${role} is growing fastest`,
      momentumDescription: (count, multiplier) =>
        `${count} postings in the last 30 days, ${multiplier} vs the previous 30-day window.`,
      compensation: "Compensation",
      compensationTitle: "Salary benchmarks are low-confidence",
      compensationDescription: (count, share, minCount) =>
        `${count} listings expose usable pay data, or ${share} of the snapshot; rankings hide groups below ${minCount} records.`,
      demand: "Demand",
      demandTitle: (role) => `${role} still anchors hiring`,
      demandDescription: (share) => `${share} of tracked vacancies fall into this role category.`,
    },
    summary: {
      vacanciesDescription: "Current public export size across the Swiss tech market.",
      employersLabel: "Direct employers",
      employersDescription: "Distinct hiring companies after excluding recruiting agencies.",
      publishedLabel: "Published in 30d",
      publishedDescription: "Latest rolling-month intake of newly published vacancies.",
      salaryCoverageDescription: (count) =>
        `${count} vacancies with normalized yearly salary data.`,
    },
    charts: {
      mapTitlePrefix: "Swiss vacancy",
      mapTitleAccent: "map",
      dashboardTitlePrefix: "Analysis",
      dashboardTitleAccent: "dashboard",
      roleShare: "Role category share",
      leadingCantons: "Leading cantons",
      workMode: "Work mode distribution",
      seniorityMix: "Seniority mix",
      topCities: "Top cities",
      topEmployers: "Top direct employers",
      topEmployersDescription: "Recruiting agencies and job boards are excluded.",
    },
    salary: {
      sectionTitlePrefix: "Salary",
      sectionTitleSeparator: " ",
      sectionTitleAccent: "metrics",
      snapshotTitle: "Compensation snapshot",
      snapshotDescription: "Comparable CHF salaries normalized to yearly values.",
      breakdownAria: "Salary breakdown",
      roles: "Roles",
      seniority: "Seniority",
      averageYearly: "Average yearly",
      medianYearly: "Median yearly",
      salaryCoverage: "Salary coverage",
      records: "Records",
      seniorityChartTitle: "Seniority ranked by average salary",
      seniorityChartDescription:
        "Seniority levels with at least 10 normalized CHF salary records.",
      roleChartTitle: "Roles ranked by average salary",
      roleChartDescription: "Role categories with at least 10 normalized CHF salary records.",
      confidenceTitle: "Low-confidence salary benchmark",
      confidenceText: (minCount, hiddenCount, coverage, count) =>
        `Ranking only shows groups with at least ${minCount} salary records.${
          hiddenCount ? ` ${hiddenCount} thin-sample groups are hidden.` : ""
        } Overall salary coverage is ${coverage} from ${count} records.`,
      median: "Median",
      noMetrics: "No comparable salary metrics available.",
      salaries: "salaries",
    },
    skills: {
      sectionTitlePrefix: "Top",
      sectionTitleMiddle: "skills",
      sectionTitleSuffix: "and",
      sectionTitleLast: "pairings",
      topOverall: "Top overall skills",
      topOverallDescription: "Ranked by vacancy frequency.",
      skill: "Skill",
      vacancies: "Vacancies",
      share: "Share",
      frequentPairings: "Frequent pairings",
      frequentPairingsDescription: "Technologies that often appear together.",
      sharedVacancies: (count) => `${count} shared vacancies`,
      topLanguages: "Top programming languages",
      topLanguagesDescription:
        "Language mentions extracted from the current vacancy snapshot.",
      coverage: "Coverage",
      distinctLanguages: "Distinct languages",
      language: "Language",
      topFrameworks: "Top frameworks & libraries",
      topFrameworksDescription:
        "Framework and library mentions extracted from the current vacancy snapshot.",
      distinctItems: "Distinct items",
      frameworkLibrary: "Framework / Library",
      jobSkillsByRole: "Job skills by role",
      jobSkillsByRoleDescription:
        "Skill mix within the leading role categories. Segment width is normalized inside each role.",
      skillLegend: "Skill legend",
      skillsByRole: "Skills by role",
      noMatrix: "No role skill matrix available.",
    },
    methodology: {
      sectionTitlePrefix: "Read",
      sectionTitleAccent: "methodology",
      sectionDescription:
        "How the public snapshot is assembled, normalized, and published for the dashboard.",
      pipelineTitle: "Pipeline overview",
      pipelineDescription:
        "The dashboard is built from aggregated vacancy exports, AI-assisted vacancy enrichment, and compact public snapshot files.",
      steps: [
        "Collect processed vacancy exports from the multi-source analytics pipeline.",
        "Deduplicate and normalize companies, locations, work mode, seniority, and salary fields.",
        "Run AI-assisted vacancy analysis to improve classification accuracy and recover details that rule-based filters can miss.",
        "Convert salary data to comparable yearly CHF ranges before aggregation.",
        "Publish compact JSON snapshots and mirrored CSV extracts for the public dashboard.",
      ],
      coverageTitle: "Snapshot coverage",
      coverageDescription: "Current build metadata for the export powering this page.",
      generated: "Generated",
      jsonSnapshots: "JSON snapshots",
      jsonDescription: "Public files published to the dashboard.",
      csvInputs: "CSV inputs",
      csvDescription: "Analytics exports available for this build.",
      missingInputs: "Missing inputs",
      downloadJson: "Download JSON snapshots",
      downloadCsv: "Download CSV exports",
    },
    navigation: {
      ariaLabel: "Page sections",
      title: "Navigate",
    },
    experience: {
      title: "Experience requirements",
      description: (minSize) =>
        `Explicit years of experience requested in vacancy text, grouped by inferred seniority. Averages by seniority require at least ${minSize} year mentions.`,
      seniorityMentionLabel: (count) => `${count} year mentions with seniority`,
      yearsMentionedLabel: (count) => `${count} mention years of experience`,
      averageMinimum: "Average minimum requested",
      medianMinimum: "Median minimum requested",
      mentionsBySeniority: "Experience mentions by seniority",
      seniority: "Seniority",
      averageMinExperience: "Avg. min exp.",
      mentions: "Mentions",
      distributionTitle: "Seniority distribution",
      distributionDescription:
        "Overall inferred seniority mix across vacancies, independent of explicit experience mentions.",
      vacancies: "Vacancies",
      share: "Share",
    },
    trends: {
      kicker: "Publication date index",
      title: (location) => `Job postings in ${location}`,
      description:
        "Role-category posting trend with canton filters, inferred closures and seasonality.",
      filtersAria: "Vacancy trend filters",
      region: "Region",
      switzerland: "Switzerland",
      professions: "Professions",
      period: "Period",
      periodAria: "Period",
      granularityAria: "Granularity",
      days: "Days",
      weeks: "Weeks",
      publishedSelection: "Published in selection",
      growthPrevious: "Growth vs previous period",
      closedDisappeared: "Closed / disappeared",
      seasonalityHighLow: "Seasonality high / low",
      seasonalityHighLowWithMonth: (month) => `Seasonality high / low: ${month}`,
      legendAria: "Profession legend",
      closedFootnote: "Closed means last seen before the latest crawl.",
      trendFootnote: "Trend uses vacancy publication dates.",
      empty: "No trend data for this selection.",
      chartTitle: "Vacancy trend by profession",
      tooltipTotal: (count) => `Total ${count}`,
      weekOf: (date) => `Week of ${date}`,
      monthNames: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
    },
    map: {
      emptyTitle: "Vacancies by Swiss city",
      emptyDescription: "No mappable Swiss city metrics available.",
      title: "Vacancies by Swiss city",
      description:
        "Red bubbles are scaled by vacancy count. Darker and larger bubbles represent stronger city concentration.",
      coverageDescription: (mapped, total, share) =>
        `${mapped} of ${total} tracked vacancies are placed on the city map (${share} coverage).`,
      summaryAria: "Mapped vacancy summary",
      cities: (count) => `${count} cities`,
      mappedVacancies: (count) => `${count} mapped vacancies`,
      multiCity: (count) => `${count} mention multiple cities and are split across bubbles.`,
      broadLocation: (count) =>
        `${count} use nationwide, regional, or remote-style location labels.`,
      unmapped: (unknown, unmatched) =>
        `${unknown} are unknown and ${unmatched} more still use labels not mapped to a city.`,
      svgTitle: "Swiss IT job vacancies by city",
      svgDescription:
        "Switzerland outline with red translucent circles over cities. Circle size and opacity increase with vacancy count.",
      markerTitle: (city, count) => `${city}: ${count} vacancies`,
      detailKicker: "City detail",
      selectCityTitle: "Select a city bubble",
      selectCityDescription:
        "Click a bubble on the map to inspect local hiring volume, share of the market, top roles, employers, and work mode mix.",
      rank: (rank) => `#${rank} mapped city`,
      vacancies: "Vacancies",
      marketShare: "Market share",
      topRoles: "Top roles",
      noRoles: "No role breakdown available.",
      topEmployers: "Top employers",
      noEmployers: "No employer breakdown available.",
      workMode: "Work mode split",
      noWorkMode: "No work mode breakdown available.",
      chooseCity: "Choose another city",
      searchHelp: "Type to search or use arrow keys.",
      searchPlaceholder: "Search city",
      noCity: "No matching city found.",
    },
  },
  de: {
    documentTitle: "Schweizer IT-Arbeitsmarkt",
    languageSwitcher: {
      label: "Sprache",
      ariaLabel: "Sprache auswählen",
    },
    common: {
      live: "Live",
      more: "Mehr",
      less: "Weniger",
      allInputsPresent: "Alle erwarteten CSV-Eingaben sind vorhanden.",
      moreFiles: (count) => `+${count} weitere`,
      na: "k.A.",
    },
    sections: {
      overview: "Überblick",
      snapshot: "Datengrundlage",
      findings: "Erkenntnisse",
      "vacancy-trends": "Trends",
      "vacancy-map": "Karte",
      charts: "Analysen",
      experience: "Erfahrung",
      salary: "Gehalt",
      skills: "Skills",
      metadata: "Methodik",
    },
    status: {
      loadingTitle: "Öffentliche Snapshots werden geladen",
      loadingMessage:
        "Schweizer IT-Marktdaten werden aus generierten JSON-Exporten geladen.",
      errorTitle: "Snapshot-Daten nicht verfügbar",
      errorFallback: "Die öffentlichen Snapshots konnten nicht geladen werden.",
      fetchError: "Öffentliche Statistik-Snapshots konnten nicht geladen werden.",
    },
    hero: {
      titleAccent: "Swiss",
      titleRest: "IT-Arbeitsmarkt",
      text:
        "Beobachte Einstellungsvolumen, Gehaltsbenchmarks und lokale Nachfragezentren anhand aktueller öffentlicher Schweizer IT-Stellendaten.",
      dashboardCta: "Dashboard ansehen",
      methodologyCta: "Methodik lesen",
      vacanciesTracked: "Erfasste Stellen",
      published30d: (count) => `${count} in 30 Tagen veröffentlicht`,
      currentCoverage: "aktuelle Marktabdeckung",
      medianSalary: "Median-Gehalt",
      salaryListings: (count) => `${count} Inserate mit Gehaltsdaten`,
      salaryCoverage: "Abdeckung der Gehaltsbenchmarks",
      topHiringCity: "Stärkste Stadt",
      activeVacancies: (count) => `${count} aktive Stellen`,
      cityDemand: "Stadtnachfrage",
    },
    snapshot: {
      kicker: "Snapshot-Kontext",
      title: "Worauf diese Seite basiert",
      description:
        "Kurzer Kontext zu Aktualität, Stichprobengrösse, Abdeckung und den Grenzen des öffentlichen Exports.",
      sources: "Quellen",
      sourceTitle: (count) => `${count} öffentliche Jobportale`,
      sourcesDescription:
        "Vor der Veröffentlichung des Aggregats auf Stellenebene dedupliziert.",
      updated: "Aktualisiert",
      sampleSize: "Stichprobe",
      directEmployers: (count) =>
        `${count} direkte Arbeitgeber nach Filterung von Agenturen.`,
      salaryCoverage: "Gehaltsabdeckung",
      salaryCoverageDescription: (count) =>
        `${count} Inserate mit normalisierten CHF-Jahresgehaltsdaten.`,
      includes: "Snapshot umfasst",
      coreSignals: "Zentrale Marktsignale",
      limitations: "Wichtigste Grenzen",
      limitationsTitle: "Vor Zahlenvergleichen lesen",
      scopeItems: [
        "Stellenvolumen",
        "Gehaltsbenchmarks",
        "Rollen- und Senioritätsmix",
        "Nachfrage nach Stadt und Kanton",
        "Arbeitsmodell-Mix",
        "Skill- und Stack-Trends",
      ],
      limitationItems: [
        "Öffentlicher aggregierter Snapshot, keine vollständige Erhebung des Schweizer Marktes.",
        "Gehaltsbenchmarks nutzen nur Inserate mit expliziten Gehaltsspannen.",
        "Gehaltsrankings blenden Gruppen mit weniger als 10 nutzbaren Gehaltsdatensätzen aus.",
        "Die Abdeckung ist auf seit 2026 veröffentlichte Stellen beschränkt.",
        "Einige Felder werden normalisiert oder KI-gestützt aus Inseratetext und strukturierten Metadaten abgeleitet.",
      ],
    },
    findings: {
      kicker: "Zentrale Erkenntnisse",
      title: "Was aktuell auffällt",
      location: "Standort",
      locationTitle: (city) => `${city} hat den grössten Anteil`,
      locationDescription: (share) => `${share} der erfassten Stellen konzentrieren sich dort.`,
      momentum: "Dynamik",
      momentumTitle: (role) => `${role} wächst am schnellsten`,
      momentumDescription: (count, multiplier) =>
        `${count} Ausschreibungen in den letzten 30 Tagen, ${multiplier} gegenüber dem vorherigen 30-Tage-Fenster.`,
      compensation: "Vergütung",
      compensationTitle: "Gehaltsbenchmarks haben geringe Sicherheit",
      compensationDescription: (count, share, minCount) =>
        `${count} Inserate enthalten nutzbare Gehaltsdaten, das sind ${share} des Snapshots; Rankings blenden Gruppen unter ${minCount} Datensätzen aus.`,
      demand: "Nachfrage",
      demandTitle: (role) => `${role} bleibt der Nachfrageanker`,
      demandDescription: (share) => `${share} der erfassten Stellen fallen in diese Rollenkategorie.`,
    },
    summary: {
      vacanciesDescription:
        "Aktuelle Grösse des öffentlichen Exports im Schweizer Tech-Markt.",
      employersLabel: "Direkte Arbeitgeber",
      employersDescription:
        "Eindeutige einstellende Unternehmen nach Ausschluss von Recruiting-Agenturen.",
      publishedLabel: "In 30 T veröffentlicht",
      publishedDescription: "Neu veröffentlichte Stellen im letzten rollierenden Monat.",
      salaryCoverageDescription: (count) =>
        `${count} Stellen mit normalisierten Jahresgehaltsdaten.`,
    },
    charts: {
      mapTitlePrefix: "Schweizer",
      mapTitleAccent: "Stellenkarte",
      dashboardTitlePrefix: "Analyse",
      dashboardTitleAccent: "Dashboard",
      roleShare: "Anteil nach Rollenkategorie",
      leadingCantons: "Führende Kantone",
      workMode: "Verteilung nach Arbeitsmodell",
      seniorityMix: "Senioritätsmix",
      topCities: "Top-Städte",
      topEmployers: "Top direkte Arbeitgeber",
      topEmployersDescription: "Recruiting-Agenturen und Jobboards sind ausgeschlossen.",
    },
    salary: {
      sectionTitlePrefix: "Gehalts",
      sectionTitleSeparator: "",
      sectionTitleAccent: "metriken",
      snapshotTitle: "Vergütungs-Snapshot",
      snapshotDescription: "Vergleichbare CHF-Gehälter, auf Jahreswerte normalisiert.",
      breakdownAria: "Gehaltsaufschluss",
      roles: "Rollen",
      seniority: "Seniorität",
      averageYearly: "Durchschnitt pro Jahr",
      medianYearly: "Median pro Jahr",
      salaryCoverage: "Gehaltsabdeckung",
      records: "Datensätze",
      seniorityChartTitle: "Seniorität nach Durchschnittsgehalt",
      seniorityChartDescription:
        "Senioritätsstufen mit mindestens 10 normalisierten CHF-Gehaltsdatensätzen.",
      roleChartTitle: "Rollen nach Durchschnittsgehalt",
      roleChartDescription:
        "Rollenkategorien mit mindestens 10 normalisierten CHF-Gehaltsdatensätzen.",
      confidenceTitle: "Gehaltsbenchmark mit geringer Sicherheit",
      confidenceText: (minCount, hiddenCount, coverage, count) =>
        `Das Ranking zeigt nur Gruppen mit mindestens ${minCount} Gehaltsdatensätzen.${
          hiddenCount ? ` ${hiddenCount} Gruppen mit kleiner Stichprobe sind ausgeblendet.` : ""
        } Die gesamte Gehaltsabdeckung beträgt ${coverage} aus ${count} Datensätzen.`,
      median: "Median",
      noMetrics: "Keine vergleichbaren Gehaltsmetriken verfügbar.",
      salaries: "Gehälter",
    },
    skills: {
      sectionTitlePrefix: "Top",
      sectionTitleMiddle: "Skills",
      sectionTitleSuffix: "und",
      sectionTitleLast: "Kombinationen",
      topOverall: "Top-Skills gesamt",
      topOverallDescription: "Nach Häufigkeit in Stellenanzeigen sortiert.",
      skill: "Skill",
      vacancies: "Stellen",
      share: "Anteil",
      frequentPairings: "Häufige Kombinationen",
      frequentPairingsDescription: "Technologien, die oft gemeinsam genannt werden.",
      sharedVacancies: (count) => `${count} gemeinsame Stellen`,
      topLanguages: "Top-Programmiersprachen",
      topLanguagesDescription:
        "Sprach-Nennungen aus dem aktuellen Stellen-Snapshot.",
      coverage: "Abdeckung",
      distinctLanguages: "Unterschiedliche Sprachen",
      language: "Sprache",
      topFrameworks: "Top-Frameworks und Libraries",
      topFrameworksDescription:
        "Framework- und Library-Nennungen aus dem aktuellen Stellen-Snapshot.",
      distinctItems: "Unterschiedliche Einträge",
      frameworkLibrary: "Framework / Library",
      jobSkillsByRole: "Job-Skills nach Rolle",
      jobSkillsByRoleDescription:
        "Skill-Mix innerhalb der führenden Rollenkategorien. Segmentbreiten sind je Rolle normalisiert.",
      skillLegend: "Skill-Legende",
      skillsByRole: "Skills nach Rolle",
      noMatrix: "Keine Rollen-Skill-Matrix verfügbar.",
    },
    methodology: {
      sectionTitlePrefix: "Methodik",
      sectionTitleAccent: "lesen",
      sectionDescription:
        "Wie der öffentliche Snapshot zusammengestellt, normalisiert und für das Dashboard veröffentlicht wird.",
      pipelineTitle: "Pipeline-Überblick",
      pipelineDescription:
        "Das Dashboard basiert auf aggregierten Stellenexporten, KI-gestützter Stellenanreicherung und kompakten öffentlichen Snapshot-Dateien.",
      steps: [
        "Verarbeitete Stellenexporte aus der Multi-Source-Analytics-Pipeline sammeln.",
        "Unternehmen, Standorte, Arbeitsmodell, Seniorität und Gehaltsfelder deduplizieren und normalisieren.",
        "KI-gestützte Stellenanalyse ausführen, um Klassifikationen zu verbessern und Details zu erfassen, die regelbasierte Filter verpassen können.",
        "Gehaltsdaten vor der Aggregation in vergleichbare Jahreswerte in CHF umrechnen.",
        "Kompakte JSON-Snapshots und gespiegelte CSV-Exporte für das öffentliche Dashboard veröffentlichen.",
      ],
      coverageTitle: "Snapshot-Abdeckung",
      coverageDescription: "Build-Metadaten des Exports, der diese Seite speist.",
      generated: "Generiert",
      jsonSnapshots: "JSON-Snapshots",
      jsonDescription: "Öffentliche Dateien, die im Dashboard veröffentlicht werden.",
      csvInputs: "CSV-Eingaben",
      csvDescription: "Analytics-Exporte, die für diesen Build verfügbar sind.",
      missingInputs: "Fehlende Eingaben",
      downloadJson: "JSON-Snapshots herunterladen",
      downloadCsv: "CSV-Exporte herunterladen",
    },
    navigation: {
      ariaLabel: "Seitenabschnitte",
      title: "Navigation",
    },
    experience: {
      title: "Erfahrungsanforderungen",
      description: (minSize) =>
        `Explizit geforderte Jahre Berufserfahrung aus dem Inseratetext, gruppiert nach abgeleiteter Seniorität. Durchschnittswerte je Seniorität benötigen mindestens ${minSize} Jahresnennungen.`,
      seniorityMentionLabel: (count) => `${count} Jahresnennungen mit Seniorität`,
      yearsMentionedLabel: (count) => `${count} Nennungen von Berufsjahren`,
      averageMinimum: "Durchschnittlich mindestens",
      medianMinimum: "Median mindestens",
      mentionsBySeniority: "Erfahrungsnennungen nach Seniorität",
      seniority: "Seniorität",
      averageMinExperience: "Durchschn. min. Erf.",
      mentions: "Nennungen",
      distributionTitle: "Senioritätsverteilung",
      distributionDescription:
        "Gesamter abgeleiteter Senioritätsmix über alle Stellen, unabhängig von expliziten Erfahrungsnennungen.",
      vacancies: "Stellen",
      share: "Anteil",
    },
    trends: {
      kicker: "Index nach Veröffentlichungsdatum",
      title: (location) => `Stellenausschreibungen in ${location}`,
      description:
        "Trend nach Rollenkategorie mit Kantonsfiltern, abgeleiteten Schliessungen und Saisonalität.",
      filtersAria: "Filter für Stellentrends",
      region: "Region",
      switzerland: "Schweiz",
      professions: "Berufe",
      period: "Zeitraum",
      periodAria: "Zeitraum",
      granularityAria: "Granularität",
      days: "Tage",
      weeks: "Wochen",
      publishedSelection: "In Auswahl veröffentlicht",
      growthPrevious: "Wachstum ggü. Vorperiode",
      closedDisappeared: "Geschlossen / verschwunden",
      seasonalityHighLow: "Saisonalität hoch / tief",
      seasonalityHighLowWithMonth: (month) => `Saisonalität hoch / tief: ${month}`,
      legendAria: "Legende der Berufe",
      closedFootnote: "Geschlossen bedeutet: vor dem letzten Crawl zuletzt gesehen.",
      trendFootnote: "Der Trend nutzt Veröffentlichungsdaten der Stellen.",
      empty: "Keine Trenddaten für diese Auswahl.",
      chartTitle: "Stellentrend nach Beruf",
      tooltipTotal: (count) => `Total ${count}`,
      weekOf: (date) => `Woche ab ${date}`,
      monthNames: ["Jan", "Feb", "Mar", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez"],
    },
    map: {
      emptyTitle: "Stellen nach Schweizer Stadt",
      emptyDescription: "Keine kartierbaren Schweizer Stadtmetriken verfügbar.",
      title: "Stellen nach Schweizer Stadt",
      description:
        "Rote Blasen sind nach Stellenzahl skaliert. Dunklere und grössere Blasen zeigen stärkere Konzentration in einer Stadt.",
      coverageDescription: (mapped, total, share) =>
        `${mapped} von ${total} erfassten Stellen sind auf der Stadtkarte platziert (${share} Abdeckung).`,
      summaryAria: "Zusammenfassung der kartierten Stellen",
      cities: (count) => `${count} Städte`,
      mappedVacancies: (count) => `${count} kartierte Stellen`,
      multiCity: (count) =>
        `${count} nennen mehrere Städte und werden auf Blasen aufgeteilt.`,
      broadLocation: (count) =>
        `${count} nutzen landesweite, regionale oder Remote-artige Standortangaben.`,
      unmapped: (unknown, unmatched) =>
        `${unknown} sind unbekannt und ${unmatched} weitere nutzen nicht kartierte Standortlabels.`,
      svgTitle: "Schweizer IT-Stellen nach Stadt",
      svgDescription:
        "Schweiz-Umriss mit roten transparenten Kreisen über Städten. Kreisgrösse und Deckkraft steigen mit der Stellenzahl.",
      markerTitle: (city, count) => `${city}: ${count} Stellen`,
      detailKicker: "Stadtdetail",
      selectCityTitle: "Stadtblase auswählen",
      selectCityDescription:
        "Klicke auf eine Blase in der Karte, um lokales Einstellungsvolumen, Marktanteil, Top-Rollen, Arbeitgeber und Arbeitsmodell-Mix zu sehen.",
      rank: (rank) => `#${rank} kartierte Stadt`,
      vacancies: "Stellen",
      marketShare: "Marktanteil",
      topRoles: "Top-Rollen",
      noRoles: "Kein Rollenaufschluss verfügbar.",
      topEmployers: "Top-Arbeitgeber",
      noEmployers: "Kein Arbeitgeberaufschluss verfügbar.",
      workMode: "Arbeitsmodell-Mix",
      noWorkMode: "Kein Arbeitsmodellaufschluss verfügbar.",
      chooseCity: "Andere Stadt auswählen",
      searchHelp: "Tippen zum Suchen oder Pfeiltasten nutzen.",
      searchPlaceholder: "Stadt suchen",
      noCity: "Keine passende Stadt gefunden.",
    },
  },
  fr: {
    documentTitle: "Marché suisse de l'emploi IT",
    languageSwitcher: {
      label: "Langue",
      ariaLabel: "Sélectionner la langue",
    },
    common: {
      live: "En direct",
      more: "Plus",
      less: "Moins",
      allInputsPresent: "Toutes les entrées CSV attendues sont présentes.",
      moreFiles: (count) => `+${count} autres`,
      na: "n.d.",
    },
    sections: {
      overview: "Aperçu",
      snapshot: "Données",
      findings: "Constats",
      "vacancy-trends": "Tendances",
      "vacancy-map": "Carte",
      charts: "Analyses",
      experience: "Expérience",
      salary: "Salaire",
      skills: "Compétences",
      metadata: "Méthodologie",
    },
    status: {
      loadingTitle: "Chargement des snapshots publics",
      loadingMessage:
        "Chargement des statistiques du marché IT suisse depuis les exports JSON générés.",
      errorTitle: "Données de snapshot indisponibles",
      errorFallback: "Les snapshots publics n'ont pas pu être chargés.",
      fetchError: "Impossible de charger les snapshots statistiques publics.",
    },
    hero: {
      titleAccent: "Marché",
      titleRest: "IT suisse",
      text:
        "Suivez le volume de recrutement, les repères salariaux et les pôles géographiques à partir des dernières données publiques d'offres IT en Suisse.",
      dashboardCta: "Explorer le dashboard",
      methodologyCta: "Lire la méthodologie",
      vacanciesTracked: "Offres suivies",
      published30d: (count) => `${count} publiées en 30 jours`,
      currentCoverage: "couverture actuelle du marché",
      medianSalary: "Salaire médian",
      salaryListings: (count) => `${count} offres avec données salariales`,
      salaryCoverage: "couverture des repères salariaux",
      topHiringCity: "Ville la plus active",
      activeVacancies: (count) => `${count} offres actives`,
      cityDemand: "demande par ville",
    },
    snapshot: {
      kicker: "Contexte du snapshot",
      title: "Base de cette page",
      description:
        "Contexte rapide sur la fraîcheur, la taille de l'échantillon, la couverture et les limites de l'export public.",
      sources: "Sources",
      sourceTitle: (count) => `${count} job boards publics`,
      sourcesDescription:
        "Dédoublonnage au niveau de l'offre avant la publication de l'agrégat public.",
      updated: "Mis à jour",
      sampleSize: "Échantillon",
      directEmployers: (count) =>
        `${count} employeurs directs après filtrage des agences.`,
      salaryCoverage: "Couverture salariale",
      salaryCoverageDescription: (count) =>
        `${count} offres avec salaires annuels CHF normalisés.`,
      includes: "Le snapshot inclut",
      coreSignals: "Signaux clés du marché",
      limitations: "Principales limites",
      limitationsTitle: "À lire avant de comparer les chiffres",
      scopeItems: [
        "Volume d'offres",
        "Repères salariaux",
        "Mix des rôles et de la seniorité",
        "Demande par ville et canton",
        "Répartition des modes de travail",
        "Tendances des compétences et stacks",
      ],
      limitationItems: [
        "Snapshot public agrégé, pas un recensement complet du marché suisse.",
        "Les repères salariaux utilisent uniquement les offres avec fourchettes explicites.",
        "Les classements salariaux masquent les groupes avec moins de 10 salaires exploitables.",
        "La couverture est limitée aux offres publiées depuis 2026.",
        "Certains champs sont normalisés ou enrichis par IA à partir du texte des offres et des métadonnées structurées.",
      ],
    },
    findings: {
      kicker: "Constats clés",
      title: "Ce qui ressort actuellement",
      location: "Localisation",
      locationTitle: (city) => `${city} conserve la plus grande part`,
      locationDescription: (share) => `${share} des offres suivies y sont concentrées.`,
      momentum: "Dynamique",
      momentumTitle: (role) => `${role} progresse le plus vite`,
      momentumDescription: (count, multiplier) =>
        `${count} publications sur les 30 derniers jours, ${multiplier} par rapport à la fenêtre précédente de 30 jours.`,
      compensation: "Rémunération",
      compensationTitle: "Les repères salariaux ont une confiance limitée",
      compensationDescription: (count, share, minCount) =>
        `${count} offres exposent des données salariales exploitables, soit ${share} du snapshot; les classements masquent les groupes sous ${minCount} enregistrements.`,
      demand: "Demande",
      demandTitle: (role) => `${role} reste le principal moteur`,
      demandDescription: (share) => `${share} des offres suivies appartiennent à cette catégorie de rôle.`,
    },
    summary: {
      vacanciesDescription: "Taille actuelle de l'export public sur le marché tech suisse.",
      employersLabel: "Employeurs directs",
      employersDescription:
        "Entreprises recruteuses distinctes après exclusion des agences de recrutement.",
      publishedLabel: "Publiées en 30 j",
      publishedDescription: "Nouvelles offres publiées sur le dernier mois glissant.",
      salaryCoverageDescription: (count) =>
        `${count} offres avec salaires annuels normalisés.`,
    },
    charts: {
      mapTitlePrefix: "Carte des offres",
      mapTitleAccent: "suisses",
      dashboardTitlePrefix: "Dashboard",
      dashboardTitleAccent: "d'analyse",
      roleShare: "Part par catégorie de rôle",
      leadingCantons: "Cantons principaux",
      workMode: "Répartition des modes de travail",
      seniorityMix: "Mix de seniorité",
      topCities: "Villes principales",
      topEmployers: "Principaux employeurs directs",
      topEmployersDescription: "Les agences de recrutement et job boards sont exclus.",
    },
    salary: {
      sectionTitlePrefix: "Métriques",
      sectionTitleSeparator: " ",
      sectionTitleAccent: "salariales",
      snapshotTitle: "Snapshot de rémunération",
      snapshotDescription: "Salaires CHF comparables, normalisés en valeurs annuelles.",
      breakdownAria: "Répartition salariale",
      roles: "Rôles",
      seniority: "Seniorité",
      averageYearly: "Moyenne annuelle",
      medianYearly: "Médiane annuelle",
      salaryCoverage: "Couverture salariale",
      records: "Enregistrements",
      seniorityChartTitle: "Seniorité classée par salaire moyen",
      seniorityChartDescription:
        "Niveaux de seniorité avec au moins 10 salaires CHF normalisés.",
      roleChartTitle: "Rôles classés par salaire moyen",
      roleChartDescription:
        "Catégories de rôle avec au moins 10 salaires CHF normalisés.",
      confidenceTitle: "Repère salarial à confiance limitée",
      confidenceText: (minCount, hiddenCount, coverage, count) =>
        `Le classement affiche uniquement les groupes avec au moins ${minCount} salaires.${
          hiddenCount ? ` ${hiddenCount} groupes à petit échantillon sont masqués.` : ""
        } La couverture salariale totale est de ${coverage} sur ${count} enregistrements.`,
      median: "Médiane",
      noMetrics: "Aucune métrique salariale comparable disponible.",
      salaries: "salaires",
    },
    skills: {
      sectionTitlePrefix: "Top",
      sectionTitleMiddle: "compétences",
      sectionTitleSuffix: "et",
      sectionTitleLast: "associations",
      topOverall: "Compétences principales",
      topOverallDescription: "Classées par fréquence dans les offres.",
      skill: "Compétence",
      vacancies: "Offres",
      share: "Part",
      frequentPairings: "Associations fréquentes",
      frequentPairingsDescription: "Technologies qui apparaissent souvent ensemble.",
      sharedVacancies: (count) => `${count} offres partagées`,
      topLanguages: "Langages de programmation principaux",
      topLanguagesDescription:
        "Mentions de langages extraites du snapshot actuel des offres.",
      coverage: "Couverture",
      distinctLanguages: "Langages distincts",
      language: "Langage",
      topFrameworks: "Frameworks et bibliothèques principaux",
      topFrameworksDescription:
        "Mentions de frameworks et bibliothèques extraites du snapshot actuel des offres.",
      distinctItems: "Éléments distincts",
      frameworkLibrary: "Framework / Bibliothèque",
      jobSkillsByRole: "Compétences par rôle",
      jobSkillsByRoleDescription:
        "Mix de compétences dans les catégories de rôle principales. La largeur des segments est normalisée dans chaque rôle.",
      skillLegend: "Légende des compétences",
      skillsByRole: "Compétences par rôle",
      noMatrix: "Aucune matrice rôle-compétence disponible.",
    },
    methodology: {
      sectionTitlePrefix: "Lire la",
      sectionTitleAccent: "méthodologie",
      sectionDescription:
        "Comment le snapshot public est assemblé, normalisé et publié pour le dashboard.",
      pipelineTitle: "Aperçu du pipeline",
      pipelineDescription:
        "Le dashboard repose sur des exports d'offres agrégés, un enrichissement assisté par IA et des fichiers de snapshot publics compacts.",
      steps: [
        "Collecter les exports d'offres traités depuis le pipeline analytique multi-source.",
        "Dédoublonner et normaliser les entreprises, localisations, modes de travail, niveaux de seniorité et champs salariaux.",
        "Exécuter une analyse assistée par IA pour améliorer la précision des classifications et récupérer les détails que les filtres à règles peuvent manquer.",
        "Convertir les données salariales en fourchettes annuelles CHF comparables avant agrégation.",
        "Publier des snapshots JSON compacts et des extraits CSV miroir pour le dashboard public.",
      ],
      coverageTitle: "Couverture du snapshot",
      coverageDescription: "Métadonnées du build qui alimente cette page.",
      generated: "Généré",
      jsonSnapshots: "Snapshots JSON",
      jsonDescription: "Fichiers publics publiés dans le dashboard.",
      csvInputs: "Entrées CSV",
      csvDescription: "Exports analytiques disponibles pour ce build.",
      missingInputs: "Entrées manquantes",
      downloadJson: "Télécharger les snapshots JSON",
      downloadCsv: "Télécharger les exports CSV",
    },
    navigation: {
      ariaLabel: "Sections de la page",
      title: "Navigation",
    },
    experience: {
      title: "Exigences d'expérience",
      description: (minSize) =>
        `Années d'expérience explicitement demandées dans le texte des offres, groupées par seniorité inférée. Les moyennes par seniorité requièrent au moins ${minSize} mentions d'années.`,
      seniorityMentionLabel: (count) => `${count} mentions d'années avec seniorité`,
      yearsMentionedLabel: (count) => `${count} mentions d'années d'expérience`,
      averageMinimum: "Minimum moyen demandé",
      medianMinimum: "Minimum médian demandé",
      mentionsBySeniority: "Mentions d'expérience par seniorité",
      seniority: "Seniorité",
      averageMinExperience: "Exp. min. moy.",
      mentions: "Mentions",
      distributionTitle: "Répartition de la seniorité",
      distributionDescription:
        "Mix global de seniorité inférée sur les offres, indépendamment des mentions explicites d'expérience.",
      vacancies: "Offres",
      share: "Part",
    },
    trends: {
      kicker: "Index par date de publication",
      title: (location) => `Offres d'emploi en ${location}`,
      description:
        "Tendance des publications par catégorie de rôle, avec filtres cantonaux, fermetures inférées et saisonnalité.",
      filtersAria: "Filtres de tendance des offres",
      region: "Région",
      switzerland: "Suisse",
      professions: "Professions",
      period: "Période",
      periodAria: "Période",
      granularityAria: "Granularité",
      days: "Jours",
      weeks: "Semaines",
      publishedSelection: "Publiées dans la sélection",
      growthPrevious: "Croissance vs période précédente",
      closedDisappeared: "Fermées / disparues",
      seasonalityHighLow: "Saisonnalité haute / basse",
      seasonalityHighLowWithMonth: (month) => `Saisonnalité haute / basse: ${month}`,
      legendAria: "Légende des professions",
      closedFootnote: "Fermée signifie vue pour la dernière fois avant le dernier crawl.",
      trendFootnote: "La tendance utilise les dates de publication des offres.",
      empty: "Aucune donnée de tendance pour cette sélection.",
      chartTitle: "Tendance des offres par profession",
      tooltipTotal: (count) => `Total ${count}`,
      weekOf: (date) => `Semaine du ${date}`,
      monthNames: [
        "Janv.",
        "Févr.",
        "Mars",
        "Avr.",
        "Mai",
        "Juin",
        "Juil.",
        "Août",
        "Sept.",
        "Oct.",
        "Nov.",
        "Déc.",
      ],
    },
    map: {
      emptyTitle: "Offres par ville suisse",
      emptyDescription: "Aucune métrique de ville suisse cartographiable disponible.",
      title: "Offres par ville suisse",
      description:
        "Les bulles rouges sont proportionnelles au nombre d'offres. Les bulles plus foncées et plus grandes indiquent une concentration plus forte.",
      coverageDescription: (mapped, total, share) =>
        `${mapped} offres sur ${total} suivies sont placées sur la carte des villes (${share} de couverture).`,
      summaryAria: "Résumé des offres cartographiées",
      cities: (count) => `${count} villes`,
      mappedVacancies: (count) => `${count} offres cartographiées`,
      multiCity: (count) =>
        `${count} mentionnent plusieurs villes et sont réparties entre les bulles.`,
      broadLocation: (count) =>
        `${count} utilisent des libellés nationaux, régionaux ou de type remote.`,
      unmapped: (unknown, unmatched) =>
        `${unknown} sont inconnues et ${unmatched} autres utilisent encore des libellés non cartographiés.`,
      svgTitle: "Offres IT suisses par ville",
      svgDescription:
        "Contour de la Suisse avec des cercles rouges translucides sur les villes. La taille et l'opacité augmentent avec le nombre d'offres.",
      markerTitle: (city, count) => `${city}: ${count} offres`,
      detailKicker: "Détail ville",
      selectCityTitle: "Sélectionner une bulle de ville",
      selectCityDescription:
        "Cliquez sur une bulle de la carte pour inspecter le volume de recrutement local, la part de marché, les rôles principaux, les employeurs et le mix des modes de travail.",
      rank: (rank) => `#${rank} ville cartographiée`,
      vacancies: "Offres",
      marketShare: "Part de marché",
      topRoles: "Rôles principaux",
      noRoles: "Aucune répartition par rôle disponible.",
      topEmployers: "Employeurs principaux",
      noEmployers: "Aucune répartition par employeur disponible.",
      workMode: "Mix des modes de travail",
      noWorkMode: "Aucune répartition des modes de travail disponible.",
      chooseCity: "Choisir une autre ville",
      searchHelp: "Tapez pour rechercher ou utilisez les flèches.",
      searchPlaceholder: "Rechercher une ville",
      noCity: "Aucune ville correspondante trouvée.",
    },
  },
  it: {
    documentTitle: "Mercato svizzero del lavoro IT",
    languageSwitcher: {
      label: "Lingua",
      ariaLabel: "Seleziona lingua",
    },
    common: {
      live: "Live",
      more: "Altro",
      less: "Meno",
      allInputsPresent: "Tutti gli input CSV previsti sono presenti.",
      moreFiles: (count) => `+${count} altri`,
      na: "n.d.",
    },
    sections: {
      overview: "Panoramica",
      snapshot: "Dati",
      findings: "Risultati",
      "vacancy-trends": "Tendenze",
      "vacancy-map": "Mappa",
      charts: "Analisi",
      experience: "Esperienza",
      salary: "Salario",
      skills: "Competenze",
      metadata: "Metodologia",
    },
    status: {
      loadingTitle: "Caricamento degli snapshot pubblici",
      loadingMessage:
        "Caricamento delle statistiche del mercato IT svizzero dagli export JSON generati.",
      errorTitle: "Dati dello snapshot non disponibili",
      errorFallback: "Gli snapshot pubblici non hanno potuto essere caricati.",
      fetchError: "Impossibile caricare gli snapshot statistici pubblici.",
    },
    hero: {
      titleAccent: "Mercato",
      titleRest: "IT svizzero",
      text:
        "Monitora volume di assunzioni, benchmark salariali e hotspot geografici dagli ultimi dati pubblici sulle offerte IT in Svizzera.",
      dashboardCta: "Esplora il dashboard",
      methodologyCta: "Leggi la metodologia",
      vacanciesTracked: "Offerte monitorate",
      published30d: (count) => `${count} pubblicate in 30 giorni`,
      currentCoverage: "copertura attuale del mercato",
      medianSalary: "Salario mediano",
      salaryListings: (count) => `${count} offerte con dati salariali`,
      salaryCoverage: "copertura dei benchmark salariali",
      topHiringCity: "Città più attiva",
      activeVacancies: (count) => `${count} offerte attive`,
      cityDemand: "domanda per città",
    },
    snapshot: {
      kicker: "Contesto dello snapshot",
      title: "Base di questa pagina",
      description:
        "Contesto rapido su freschezza, dimensione del campione, copertura e limiti dell'export pubblico.",
      sources: "Fonti",
      sourceTitle: (count) => `${count} job board pubbliche`,
      sourcesDescription:
        "Deduplicazione a livello di offerta prima della pubblicazione dell'aggregato pubblico.",
      updated: "Aggiornato",
      sampleSize: "Dimensione del campione",
      directEmployers: (count) =>
        `${count} datori di lavoro diretti dopo il filtro delle agenzie.`,
      salaryCoverage: "Copertura salariale",
      salaryCoverageDescription: (count) =>
        `${count} offerte con dati salariali annui CHF normalizzati.`,
      includes: "Lo snapshot include",
      coreSignals: "Segnali chiave del mercato",
      limitations: "Limiti principali",
      limitationsTitle: "Da leggere prima di confrontare i numeri",
      scopeItems: [
        "Volume delle offerte",
        "Benchmark salariali",
        "Mix di ruoli e seniority",
        "Domanda per città e cantone",
        "Ripartizione delle modalità di lavoro",
        "Tendenze di competenze e stack",
      ],
      limitationItems: [
        "Snapshot pubblico aggregato, non un censimento completo del mercato svizzero.",
        "I benchmark salariali usano solo offerte con intervalli espliciti.",
        "Le classifiche salariali nascondono i gruppi con meno di 10 salari utilizzabili.",
        "La copertura è limitata alle offerte pubblicate dal 2026.",
        "Alcuni campi sono normalizzati o arricchiti con IA a partire dal testo delle offerte e dai metadati strutturati.",
      ],
    },
    findings: {
      kicker: "Risultati chiave",
      title: "Cosa emerge al momento",
      location: "Località",
      locationTitle: (city) => `${city} mantiene la quota più alta`,
      locationDescription: (share) => `${share} delle offerte monitorate è concentrato qui.`,
      momentum: "Dinamica",
      momentumTitle: (role) => `${role} cresce più rapidamente`,
      momentumDescription: (count, multiplier) =>
        `${count} pubblicazioni negli ultimi 30 giorni, ${multiplier} rispetto alla finestra precedente di 30 giorni.`,
      compensation: "Retribuzione",
      compensationTitle: "I benchmark salariali hanno una confidenza limitata",
      compensationDescription: (count, share, minCount) =>
        `${count} offerte espongono dati salariali utilizzabili, pari al ${share} dello snapshot; le classifiche nascondono i gruppi sotto ${minCount} record.`,
      demand: "Domanda",
      demandTitle: (role) => `${role} resta il principale driver`,
      demandDescription: (share) => `${share} delle offerte monitorate appartiene a questa categoria di ruolo.`,
    },
    summary: {
      vacanciesDescription: "Dimensione attuale dell'export pubblico sul mercato tech svizzero.",
      employersLabel: "Datori diretti",
      employersDescription:
        "Aziende che assumono direttamente dopo l'esclusione delle agenzie di recruiting.",
      publishedLabel: "Pubblicate in 30 g",
      publishedDescription: "Nuove offerte pubblicate nell'ultimo mese mobile.",
      salaryCoverageDescription: (count) =>
        `${count} offerte con salari annui normalizzati.`,
    },
    charts: {
      mapTitlePrefix: "Mappa delle offerte",
      mapTitleAccent: "svizzere",
      dashboardTitlePrefix: "Dashboard",
      dashboardTitleAccent: "analitico",
      roleShare: "Quota per categoria di ruolo",
      leadingCantons: "Cantoni principali",
      workMode: "Ripartizione modalità di lavoro",
      seniorityMix: "Mix di seniority",
      topCities: "Città principali",
      topEmployers: "Principali datori diretti",
      topEmployersDescription: "Agenzie di recruiting e job board sono esclusi.",
    },
    salary: {
      sectionTitlePrefix: "Metriche",
      sectionTitleSeparator: " ",
      sectionTitleAccent: "salariali",
      snapshotTitle: "Snapshot retributivo",
      snapshotDescription: "Salari CHF comparabili, normalizzati in valori annui.",
      breakdownAria: "Ripartizione salariale",
      roles: "Ruoli",
      seniority: "Seniority",
      averageYearly: "Media annua",
      medianYearly: "Mediana annua",
      salaryCoverage: "Copertura salariale",
      records: "Record",
      seniorityChartTitle: "Seniority ordinate per salario medio",
      seniorityChartDescription:
        "Livelli di seniority con almeno 10 salari CHF normalizzati.",
      roleChartTitle: "Ruoli ordinati per salario medio",
      roleChartDescription:
        "Categorie di ruolo con almeno 10 salari CHF normalizzati.",
      confidenceTitle: "Benchmark salariale a confidenza limitata",
      confidenceText: (minCount, hiddenCount, coverage, count) =>
        `La classifica mostra solo i gruppi con almeno ${minCount} salari.${
          hiddenCount ? ` ${hiddenCount} gruppi con campione ridotto sono nascosti.` : ""
        } La copertura salariale totale è ${coverage} su ${count} record.`,
      median: "Mediana",
      noMetrics: "Nessuna metrica salariale comparabile disponibile.",
      salaries: "salari",
    },
    skills: {
      sectionTitlePrefix: "Top",
      sectionTitleMiddle: "competenze",
      sectionTitleSuffix: "e",
      sectionTitleLast: "associazioni",
      topOverall: "Competenze principali",
      topOverallDescription: "Classificate per frequenza nelle offerte.",
      skill: "Competenza",
      vacancies: "Offerte",
      share: "Quota",
      frequentPairings: "Associazioni frequenti",
      frequentPairingsDescription: "Tecnologie che compaiono spesso insieme.",
      sharedVacancies: (count) => `${count} offerte condivise`,
      topLanguages: "Linguaggi di programmazione principali",
      topLanguagesDescription:
        "Menzioni di linguaggi estratte dallo snapshot attuale delle offerte.",
      coverage: "Copertura",
      distinctLanguages: "Linguaggi distinti",
      language: "Linguaggio",
      topFrameworks: "Framework e librerie principali",
      topFrameworksDescription:
        "Menzioni di framework e librerie estratte dallo snapshot attuale delle offerte.",
      distinctItems: "Elementi distinti",
      frameworkLibrary: "Framework / Libreria",
      jobSkillsByRole: "Competenze per ruolo",
      jobSkillsByRoleDescription:
        "Mix di competenze nelle principali categorie di ruolo. La larghezza dei segmenti è normalizzata all'interno di ogni ruolo.",
      skillLegend: "Legenda competenze",
      skillsByRole: "Competenze per ruolo",
      noMatrix: "Nessuna matrice ruolo-competenza disponibile.",
    },
    methodology: {
      sectionTitlePrefix: "Leggi la",
      sectionTitleAccent: "metodologia",
      sectionDescription:
        "Come lo snapshot pubblico viene assemblato, normalizzato e pubblicato per il dashboard.",
      pipelineTitle: "Panoramica della pipeline",
      pipelineDescription:
        "Il dashboard si basa su export aggregati delle offerte, arricchimento assistito da IA e snapshot pubblici compatti.",
      steps: [
        "Raccogliere gli export di offerte elaborati dalla pipeline analitica multi-source.",
        "Deduplicare e normalizzare aziende, località, modalità di lavoro, livelli di seniority e campi salariali.",
        "Eseguire un'analisi assistita da IA per migliorare la precisione delle classificazioni e recuperare dettagli che i filtri a regole possono perdere.",
        "Convertire i dati salariali in intervalli annui CHF comparabili prima dell'aggregazione.",
        "Pubblicare snapshot JSON compatti ed estratti CSV speculari per il dashboard pubblico.",
      ],
      coverageTitle: "Copertura dello snapshot",
      coverageDescription: "Metadati del build che alimenta questa pagina.",
      generated: "Generato",
      jsonSnapshots: "Snapshot JSON",
      jsonDescription: "File pubblici pubblicati nel dashboard.",
      csvInputs: "Input CSV",
      csvDescription: "Export analitici disponibili per questo build.",
      missingInputs: "Input mancanti",
      downloadJson: "Scarica snapshot JSON",
      downloadCsv: "Scarica export CSV",
    },
    navigation: {
      ariaLabel: "Sezioni della pagina",
      title: "Navigazione",
    },
    experience: {
      title: "Requisiti di esperienza",
      description: (minSize) =>
        `Anni di esperienza richiesti esplicitamente nel testo delle offerte, raggruppati per seniority inferita. Le medie per seniority richiedono almeno ${minSize} menzioni di anni.`,
      seniorityMentionLabel: (count) => `${count} menzioni di anni con seniority`,
      yearsMentionedLabel: (count) => `${count} menzioni di anni di esperienza`,
      averageMinimum: "Minimo medio richiesto",
      medianMinimum: "Minimo mediano richiesto",
      mentionsBySeniority: "Menzioni di esperienza per seniority",
      seniority: "Seniority",
      averageMinExperience: "Esp. min. media",
      mentions: "Menzioni",
      distributionTitle: "Distribuzione della seniority",
      distributionDescription:
        "Mix complessivo della seniority inferita sulle offerte, indipendentemente dalle menzioni esplicite di esperienza.",
      vacancies: "Offerte",
      share: "Quota",
    },
    trends: {
      kicker: "Indice per data di pubblicazione",
      title: (location) => `Offerte di lavoro in ${location}`,
      description:
        "Tendenza delle pubblicazioni per categoria di ruolo, con filtri cantonali, chiusure inferite e stagionalità.",
      filtersAria: "Filtri delle tendenze delle offerte",
      region: "Regione",
      switzerland: "Svizzera",
      professions: "Professioni",
      period: "Periodo",
      periodAria: "Periodo",
      granularityAria: "Granularità",
      days: "Giorni",
      weeks: "Settimane",
      publishedSelection: "Pubblicate nella selezione",
      growthPrevious: "Crescita vs periodo precedente",
      closedDisappeared: "Chiuse / scomparse",
      seasonalityHighLow: "Stagionalità alta / bassa",
      seasonalityHighLowWithMonth: (month) => `Stagionalità alta / bassa: ${month}`,
      legendAria: "Legenda delle professioni",
      closedFootnote: "Chiusa significa vista per l'ultima volta prima dell'ultimo crawl.",
      trendFootnote: "La tendenza usa le date di pubblicazione delle offerte.",
      empty: "Nessun dato di tendenza per questa selezione.",
      chartTitle: "Tendenza delle offerte per professione",
      tooltipTotal: (count) => `Totale ${count}`,
      weekOf: (date) => `Settimana del ${date}`,
      monthNames: [
        "Gen.",
        "Feb.",
        "Mar.",
        "Apr.",
        "Mag.",
        "Giu.",
        "Lug.",
        "Ago.",
        "Set.",
        "Ott.",
        "Nov.",
        "Dic.",
      ],
    },
    map: {
      emptyTitle: "Offerte per città svizzera",
      emptyDescription: "Nessuna metrica di città svizzera mappabile disponibile.",
      title: "Offerte per città svizzera",
      description:
        "Le bolle rosse sono proporzionali al numero di offerte. Bolle più scure e più grandi indicano una concentrazione più forte.",
      coverageDescription: (mapped, total, share) =>
        `${mapped} offerte su ${total} monitorate sono posizionate sulla mappa delle città (${share} di copertura).`,
      summaryAria: "Riepilogo delle offerte mappate",
      cities: (count) => `${count} città`,
      mappedVacancies: (count) => `${count} offerte mappate`,
      multiCity: (count) =>
        `${count} menzionano più città e sono distribuite tra le bolle.`,
      broadLocation: (count) =>
        `${count} usano etichette nazionali, regionali o remote.`,
      unmapped: (unknown, unmatched) =>
        `${unknown} sono sconosciute e ${unmatched} altre usano ancora etichette non mappate.`,
      svgTitle: "Offerte IT svizzere per città",
      svgDescription:
        "Contorno della Svizzera con cerchi rossi traslucidi sulle città. Dimensione e opacità aumentano con il numero di offerte.",
      markerTitle: (city, count) => `${city}: ${count} offerte`,
      detailKicker: "Dettaglio città",
      selectCityTitle: "Seleziona una bolla città",
      selectCityDescription:
        "Clicca una bolla sulla mappa per analizzare volume di recruiting locale, quota di mercato, ruoli principali, datori e mix delle modalità di lavoro.",
      rank: (rank) => `#${rank} città mappata`,
      vacancies: "Offerte",
      marketShare: "Quota di mercato",
      topRoles: "Ruoli principali",
      noRoles: "Nessuna ripartizione per ruolo disponibile.",
      topEmployers: "Datori principali",
      noEmployers: "Nessuna ripartizione per datore disponibile.",
      workMode: "Mix modalità di lavoro",
      noWorkMode: "Nessuna ripartizione delle modalità di lavoro disponibile.",
      chooseCity: "Scegli un'altra città",
      searchHelp: "Digita per cercare o usa le frecce.",
      searchPlaceholder: "Cerca una città",
      noCity: "Nessuna città corrispondente trovata.",
    },
  },
};

export function normalizeLanguage(value) {
  const normalizedValue = String(value ?? "").toLocaleLowerCase("en-US").slice(0, 2);
  return LANGUAGE_OPTIONS.some((option) => option.code === normalizedValue)
    ? normalizedValue
    : null;
}

export function getCopy(language) {
  return LANGUAGE_COPY[normalizeLanguage(language) ?? DEFAULT_LANGUAGE];
}

export function getLocale(language) {
  return LANGUAGE_LOCALES[normalizeLanguage(language) ?? DEFAULT_LANGUAGE];
}

export function getInitialLanguage() {
  if (typeof window === "undefined") {
    return DEFAULT_LANGUAGE;
  }

  const searchLanguage = normalizeLanguage(new URLSearchParams(window.location.search).get("lang"));
  if (searchLanguage) {
    return searchLanguage;
  }

  const savedLanguage = normalizeLanguage(window.localStorage.getItem("swiss-it-jobs-language"));
  if (savedLanguage) {
    return savedLanguage;
  }

  return normalizeLanguage(window.navigator.language) ?? DEFAULT_LANGUAGE;
}

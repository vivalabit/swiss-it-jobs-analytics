...
from swiss_jobs.providers.jobs_ch.backfill_salary import backfill_database
...
class JobsChBackfillSalaryTests(unittest.TestCase):
    def test_backfill_updates_existing_jobs_ch_vacancy(self) -> None:
        vacancy = VacancyFull(
            id="old-jobs-ch-1",
            ...
            is_new=False,
            url="https://www.jobs.ch/en/vacancies/detail/old-jobs-ch-1/",
            source="jobs.ch",
            raw={},
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(120000, row[0])
            self.assertEqual(150000, row[1])
            self.assertEqual("CHF", row[2])
            self.assertEqual("YEAR", row[3])
            self.assertEqual("CHF 120000-150000 / year", row[4])
